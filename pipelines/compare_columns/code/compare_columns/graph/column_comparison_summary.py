...
def column_comparison_summary(spark: SparkSession, in0: DataFrame, in1: DataFrame) -> DataFrame:
    ...
    for vColumn in set(in0.columns).difference({"EmployeeID"}):
    ...
    return df1\
        ...
        .join(
          ...
          reduce(
            lambda a, c: a & c,
            [col("exploded1.column_name") == col("exploded2.column_name"),              col("exploded1.EmployeeID") == col("exploded2.EmployeeID")],
            lit(True)
          ),
          "full_outer"
        )\
        .select(
          ...
          coalesce(col("exploded1.EmployeeID"), col("exploded2.EmployeeID")).alias("EmployeeID"), 
          col(
              "exploded1.##value##"
            )\
            .alias(
        ...
        .union(
          df3\
            ...
            .withColumn(
              "##row_number###",
              row_number().over(Window.partitionBy(col("column_name"), col("EmployeeID")).orderBy(col("EmployeeID")))
            )\
            .filter(
              (
                col("##row_number###")
        ...
        .agg(
          ...
          first(
              when(coalesce(col("mismatch_example_left"), col("mismatch_example_right")).isNotNull(), col("EmployeeID"))\
                .otherwise(lit(None)),
              ignorenulls = True
            )\
            .alias("mismatch_example_EmployeeID")
        )\
